...
struct HTreeHasher {
    ...
    static func hash(name: String, hashType: Superblock.HashVersion, hashSeed: [UInt32]) -> UInt64? {
        var major: UInt32 = 0
        var minor: UInt32 = 0
        var seed = hashSeed
        let result = name.withCString { strPtr in
            seed.withUnsafeMutableBufferPointer { seedBuf in
                ...
                return ext2_htree_hash(strPtr, Int32(name.lengthOfBytes(using: .utf8)), seedPtr, Int32(hashType.rawValue), &major, &minor)
            }
        }
